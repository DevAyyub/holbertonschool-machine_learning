...
class Dataset:
    ...
    def __init__(self, batch_size, max_len):
        """Initializes the dataset, tokenizers, and sets up the pipeline"""
        ...
        def filter_max_length(pt, en, max_length=max_len):
            """Filters out examples with more than max_len tokens"""
            return tf.logical_and(tf.size(pt) <= max_length,
                                  tf.size(en) <= max_length)

        # Pipeline for data_train
