#!/usr/bin/env python3
"""
Module for building a decision tree with Gini impurity splitting
"""
import numpy as np
...
class Decision_Tree():
    ...
    def get_gini_impurity(self, sub_population):
        """Calculates the Gini impurity of a sub-population"""
    ...
    def Gini_split_criterion(self, node):
        """Finds the best split based on Gini impurity"""
        X = self.explanatory[node.sub_population]
        best_gini = 1
        best_feature = None
        best_threshold = None

        for feature in range(self.explanatory.shape[1]):
            # Get unique thresholds based on actual data in the node
            ...
            for threshold in thresholds:
                ...
                g_left = self.get_gini_impurity(left_pop)
                g_right = self.get_gini_impurity(right_pop)
                
                # Weight by population size
                n_total = np.sum(node.sub_population)
                current_gini = (np.sum(left_pop) * g_left +
                                np.sum(right_pop) * g_right) / n_total
