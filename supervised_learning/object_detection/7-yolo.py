...
class Yolo:
    ...
    def predict(self, folder_path):
        """
        Make predictions for images in a folder.
        ...
        # Predict on the entire batch of images at once
        outputs = self.model.predict(pimages)

        predictions = []
        for i in range(len(images)):
            # Extract the specific outputs for the current image
            ...
            # Filter the low-confidence boxes
            ...
            # Display the predictions using the basename of the image path
            file_name = os.path.basename(image_paths[i])
            self.show_boxes(images[i], box_preds, pred_classes,
